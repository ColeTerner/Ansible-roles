...
def on_ping_response(response):
    body = response.body
    try:
        parsed_body = json.loads(tornado.escape.to_unicode(body))["data"]
        global monitoring
        global min_score
    ...
    try:
        ...
        for camera in cameras:
            ...
            if ffmpeg_output is None:
                ...
                try:
                    ffprobe_output = subprocess.check_output('ffprobe -v error -show_entries stream=width,height,bit_rate -of default=noprint_wrappers=1 test.mp4', shell=True)
...
def get_roi_from_cam(camera):
    gpu_settings = camera.get('stream_settings_gpu')
    if gpu_settings is None:
        return None, None, None, None
    roi = gpu_settings.get('roi')
    if len(roi) == 0:
        return None, None, None, None
...
def update_cameras():
    ...
    try:
        ...
        current_cameras = set(
            map(lambda x: (x['id'], x['stream_url'], get_filter_settings(x), get_roi_from_cam(x)), parsed))
        t = get_cameras(client)
        new_cameras = set(map(lambda x: (x['cameraGuid'], x['cameraLink'], (min_score, min_face_size, max_face_size),
                                         get_roi_from_unlift(x)), json.loads(t.decode('utf8'))))
        ...
        for item in new_cameras.difference(current_cameras):
            ...
            if item[2] != (None, None, None):
                ...
                if qual:
                    body['stream_settings_gpu']['filter_min_quality'] = qual * -1
                if min_f:
...
class IntervalThread(Thread):
    def __init__(self, func, interval, event=Event()):
...
def send_visit(image, data, face_id, confidence, gender):
...
def store_face(cropped_img, score, direction_score,
    ...
    global min_score
    global min_direction_score
    if min_score and score != 0 and abs(score) > min_score or min_direction_score and direction_score != 0 and abs(
...
class ForwardingRequestHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if self.request.uri == "/v2/face":
            ...
            detector_params = json.loads(tornado.escape.to_unicode(args["detectorParams"][0]))
            ...
            features = json.loads(tornado.escape.to_unicode(args['features'][0]))
            ...
            identify_result = json.loads(tornado.escape.to_unicode(args['identify_result'][0]))
            ...
            if face_id and should_send and (threshold is None or (confidence is not None and (confidence >= threshold))):
                try:
                    ...
                    print(data)
                    send_visit(file, data, face_id, confidence, gender)
                    print("/---- visitFace ----")
                except Exception as e:
                    print("Error: " + str(e))
            else:
                try:
                    if monitoring:
                        ...
                        print(data)
                        file = ('file', 'image.jpg', frame_img_arr)
                        send_visit(file, data, None, None, None)
...
def start_daemon_thread(func, interval):
    daemon_thread = IntervalThread(func, interval)
...
if __name__ == "__main__":
    ...
    should_play_sound = True if config['General'].get('sound', None) == 'True' else False
    unlift_host = 'unlift.ru' if not disable_ssl else '78.47.94.52'
    ...
    print("* should_play_sound = " + str(should_play_sound))
    print("//== Settings ====")
    ...
    start_daemon_thread(get_and_send_stream_stats, 3600)
    app = make_app()
    app.listen(8888)
